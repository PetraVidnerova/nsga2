...
class Plotter():
    ...
    def __plot_front(self, front, generation_number):
        filename = "{}/generation{}.png".format(self.directory,
        ...
        perfect_pareto_front_f1, perfect_pareto_front_f2 = \
            self.problem.perfect_pareto_front()
        axes.plot(perfect_pareto_front_f1, perfect_pareto_front_f2,
                  'r.', label='Perfect Pareto Front')

        if self._f1_lim is None:
            self._f1_lim = (
                min(0, min(perfect_pareto_front_f1)), max(computed_f1))
        if self._f2_lim is None:
            self._f2_lim = (
                min(0, min(perfect_pareto_front_f2)), max(computed_f2))
        ...
        axes.set_title(
            'Computed Pareto front @ Generation {:}'.format(generation_number))
